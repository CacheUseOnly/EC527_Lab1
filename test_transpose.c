...
#define BLOCK_SIZE 64 /* The block size */
...
int main(int argc, char *argv[])
{
  ...
  for (x = 0; x < NUM_TESTS && (n = A * x * x + B * x + C, n < alloc_size); x++)
  {
    ...
    set_row_length(v1, n);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    placeholder = transpose_ij_block(v0, v1, BLOCK_SIZE);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
  ...
  for (x = 0; x < NUM_TESTS && (n = A * x * x + B * x + C, n < alloc_size); x++)
  {
    ...
    set_row_length(v1, n);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    placeholder = transpose_ji_block(v0, v1, BLOCK_SIZE);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }
  OPTION++;
  for (x = 0; x < NUM_TESTS && (n = A * x * x + B * x + C, n < alloc_size); x++)
  {
    ...
    set_row_length(v1, n);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    placeholder = transpose_ij(v0, v1, BLOCK_SIZE);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }
  OPTION++;
  for (x = 0; x < NUM_TESTS && (n = A * x * x + B * x + C, n < alloc_size); x++)
  {
    ...
    set_row_length(v1, n);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    placeholder = transpose_ji(v0, v1, BLOCK_SIZE);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }

  printf("row_len, ij_b, ji_b, ij, ji\n");
  {
    int i, j;
    for (i = 0; i < x; i++)
    {
...
int transpose_ji_block(array_ptr v, array_ptr w, long int bsize)
{
  long int length = get_row_length(v);
  long int i, j, b;
  data_t *datav = get_array_start(v);
  data_t *dataw = get_array_start(w);
  for (b = 0; b < length; b += bsize)
  {
    for (j = 0; j < length; j++)
    {
      for (i = b; i < (b + bsize) && i < length; i++)
      {
        dataw[i * length + j] = datav[j * length + i];
        // w[i * length + j] = v[j * length + i];
      }
    }
  }
  return 0;
...
int transpose_ij_block(array_ptr v, array_ptr w, long int bsize)
{
  long int length = get_row_length(v);
  long int i, j, b;
  data_t *datav = get_array_start(v);
...
int transpose_ji(array_ptr v, array_ptr w, long int bsize)
{
  long int length = get_row_length(v);
  long int i, j;
  data_t *datav = get_array_start(v);
...
int transpose_ij(array_ptr v, array_ptr w, long int bsize)
{
  long int length = get_row_length(v);
  long int i, j;
  data_t *datav = get_array_start(v);
